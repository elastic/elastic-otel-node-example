...
const indexTemplate = fs.readFileSync(
    path.join(__dirname, 'views', 'index.mustache'),
    'utf8'
);
...
function normalizeShortname(rawShortname) {
    ...
    if (!VALID_SHORTNAME.test(normShortname)) {
        throw new Error(
            'invalid shortname: must match /[A-Za-z0-9_-]+/, cannot start or end with a hyphen'
        );
    }
    normShortname = normShortname.toLowerCase().replace(/-/g, '');
    if (normShortname.length > MAX_SHORTNAME_LEN) {
        throw new Error(
            `invalid shortname: too long (max ${MAX_SHORTNAME_LEN} chars)`
        );
    }
...
router.get('/:shortname', async (req, res, next) => {
    ...
    const q = await db.query('SELECT * FROM shortlinks WHERE shortname = $1', [
        shortname,
    ]);
    if (q.rowCount === 0) {
        res.status(404);
...
app.use(function onError(err, req, res, next) {
    ...
    if (err.code === '42P01') {
        ...
        advice(
            'Did `scripts/db-setup.js` fail when running `npm run db:start`?'
        );
...
const server = app.listen(PORT, HOSTNAME, function () {
    ...
    console.log(
        `  curl ${baseUrl}/ -X POST -d shortname=el -d url=https://elastic.co`
    );
